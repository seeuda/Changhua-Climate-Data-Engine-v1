...
const climateGridManager = new ClimateGridManager(datasetRegistry);
const spatialQueryEngine = new SpatialQueryEngine();
...
function loadData() {
    ...
    Promise.all([
        fetch(`data/changhua_towns.json?t=${new Date().getTime()}`).then(res => res.json()),
...
async function updateLayers() {
        ...
        wraLayer = L.geoJSON(activeWraData, {
            ...
            style: (feature) => {
                ...
                return {
                    ...
                    fillOpacity: isFloodGridRiskMode() ? floodGridOpacity : 0.65,
    ...
    // 3. Add Town Polygons (Transparent if Climate Grid is active, else original logic)
...
function onEachPointFeature(feature, layer, config) {
    ...
    let warningHtml = '';
    const wraRisk = getWraPointRisk(feature, config);
    if (activeTheme === 'flood' && isWraLayerEnabled() && wraRisk) {
        warningHtml = `
...
function renderWraScenarioSelector() {
    const selector = document.getElementById('wra-scenario-selector');
    if (!selector) return;

    selector.innerHTML = getAvailableWraScenarios().map(scenario => {
        const isActive = getActiveWraScenario() === scenario.id;
        return `
...
function updateRiskOpacityControl() {
    ...
    const gridOpacityLabel = document.getElementById('grid-opacity-label');
    ...
    if (gridOpacityGroup) {
        gridOpacityGroup.style.display = (isClimateGridRiskMode() || isFloodGridRiskMode()) ? 'flex' : 'none';
    }
    ...
    if (gridOpacityValue) {
        gridOpacityValue.innerText = `${Math.round((isFloodGridRiskMode() ? floodGridOpacity : climateGridOpacity) * 100)}%`;
    }

    if (gridOpacitySlider) {
        gridOpacitySlider.value = String(isFloodGridRiskMode() ? floodGridOpacity : climateGridOpacity);
    }
}
...
function setupUIControls() {
    ...
    if (wraScenarioSelector) {
        renderWraScenarioSelector();
        wraScenarioSelector.addEventListener('click', (e) => {
            const button = e.target.closest('[data-wra-scenario]');
            if (!button) return;
            activeWraScenario = button.dataset.wraScenario;
    ...
    if (gridOpacitySlider) {
        gridOpacitySlider.addEventListener('input', (e) => {
            if (isFloodGridRiskMode()) {
                floodGridOpacity = parseFloat(e.target.value);
            } else {
