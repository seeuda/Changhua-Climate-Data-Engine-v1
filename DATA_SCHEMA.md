...
- 高溫點位外框會依目前啟用的氣候風險圖層與情境，優先使用 NCDR AR6 氣候網格；行政區回退情境使用前端即時計算的行政區加權危害度。鄉鎮資料包中若仍存在 `temp_risk_*`，視為舊版相容欄位，前端不再讀取，避免未隨新版網格更新時被誤用。若資料的 `town` 文字註記與座標套疊結果不同，popup 會顯示差異並以座標套疊結果為準。
- 淹水點位外框不套用鄉鎮／行政區彙整風險；NCDR 淹水風險圖僅供策略層級與視覺參考。啟用水利署淹水潛勢時，系統會先做點位與潛勢多邊形的直接套疊；若點位未落入潛勢面，但距離最近潛勢面邊界 100 公尺內，會以距離反比加權（`1 - 距離 / 100m`）列為「鄰近淹水潛勢」，並沿用最近潛勢面的淹水深度級距。直接套疊或鄰近加權命中的點位都會以紅色警戒外框標示；未命中水利署圖資時維持低風險註記，不回退行政區風險。
